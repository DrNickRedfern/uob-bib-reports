...
df_publications <- read_csv(here("data", "faculty_of_health_sciences_publications_details.csv"), 
                   show_col_types = FALSE)
...
Please note: *currently affiliated researchers* are defined as researchers whose researcher id is matched to the University of Bradford's organisation id at the time the result set was generated.
...
Please note: open access refers to journal articles *only*. In the future open access requirements will apply to all type of research outputs and the metric will be updated once this change is implemented. 
...
```{r fig-outputs-publishers, fig.align='center', fig.width=6, fig.height=3.5, fig.pos='H', fig.cap=glue("Publishers of journal articles produced by researchers in the {params$research_unit}, {params$min_year} - {params$max_year}.")}
df_publications %>%
  filter(type == "article") %>%
  ...
  scale_y_continuous(name = NULL, expand = c(0, 0.1)) +
  coord_flip() +
  my_theme() +
  theme(axis.line.x = element_blank(),
        axis.line.y = element_line(colour = "gray"),
...
Of the research outputs in the result set, **`r outputs_with_at_least_one_rcdc`** are allocated at least one RCDC category, with **`r n_unique_rcdc` unique categories** used across all research outputs. Table \@ref(tab:tbl-rcdc) presents those RCDC categories which account for at least 1% of the total categories.
...
ggplot(data = impact_key_stats, 
  ...
  theme(plot.margin = unit(c(-0.30, 0 , 0, 0), "null"),
        plot.caption = element_text(size = 9)) +  # remove margin around plot
  theme_void() +
  guides(fill = "none")
```
...
```{r fig-fcr, fig.height=4, fig.pos="t", fig.cap=glue("The distribution of Field Citation Ratio scores for research outputs (n = {n_outputs_with_fcr}) published by the {params$research_unit} in {params$min_year}.")}
...
Figure \@ref(fig:fig-citation-rate) plots the distribution of the citation rates for outputs produced by researchers from the `r params$research_unit`. The median citation rate is **`r citation_rate_summary$median`**, with a range of `r citation_rate_summary$min` to `r citation_rate_summary$max`.
...
Figure \@ref(fig:fig-cpr) plots the distribution of the co-Citation Percentile Ranks for outputs produced by researchers from the `r params$research_unit`. The median CPR is **`r cpr_summary$median`**, with a range of `r cpr_summary$min` to `r cpr_summary$max`.
...
df_citing_publications <- read_csv(here("citing_publications", 
                                        "faculty_of_health_sciences_citing_pubs_details.csv"), 
                                   show_col_types = FALSE)

df_citers <- read_csv(here("citing_publications",
                           "faculty_of_health_sciences_citing_pubs_citers.csv"), 
                      show_col_types = FALSE) %>%
...
knitr::kable(df_citing_journals, align = "cl", 
             booktabs = TRUE, linesep = "\\addlinespace", longtable = TRUE,
             caption = glue("Number of citing publications for journals referencing research outputs produced by researchers from the {params$research_unit}, {params$min_year} to {params$max_year}.")) %>%
  kableExtra::kable_styling(font_size = 9.5, full_width = FALSE, 
                            latex_options = c("striped", "HOLD_position", "repeat_header")) %>%
...
```{r fig-citing-publishers, fig.align='center', fig.pos='H', fig.width=6, fig.height=5, fig.cap=glue("Number of citing publications by publisher for publications referencing research outputs produced by researchers from the {params$research_unit}, {params$min_year} to {params$max_year}.")}
...
df_citing_types %>%
  ggplot(aes(x = 2, y = count, fill = types)) +
  geom_col(colour = "black", linewidth = 0.5) +
  geom_label_repel(aes(y = pos,
                   ...
                   colour = c(rep("white", 4), "black"),
                   show.legend = FALSE) +
  scale_x_discrete(name = "Type of organisation") +
  scale_y_continuous(name = "Frequency") +
  scale_fill_manual(name = NULL, values = discrete_pal[c(9,8,7,5,3)]) +
  coord_polar(theta = "y", start = 0, direction = -1) +
  xlim(0.1, 3) +
...
Please note: *collaboration* here refers to outputs where at least one author is *not* from the University of Bradford.
...
```{r fig-collab-summary, fig.align='center', fig.width = 6, fig.height = 4, fig.pos='H', fig.cap=glue("Research outputs by collaboration type for the {params$research_unit}, {params$min_year} - {params$max_year}.")}
...
```{r fig-collab-cumulative-types, fig.align='center', fig.width = 6, fig.height = 4, fig.pos='H', fig.cap=glue("Cumualtive research outputs by collaboration type for the {params$research_unit}, {params$min_year} - {params$max_year}.")}
...
ggplot(data = top_15_orgs, aes(x = organisation_name, y = count, fill = colour)) +
  ...
  scale_fill_identity(name = "Type of research organisation",
                      labels = top_15_orgs$organisation_type,
                      breaks = top_15_orgs$colour,
                      guide = (guide_legend(title.position = "top"))) +
  coord_flip() +
  theme_light() +
  theme(axis.text.x = element_blank(),
...
ggplot(data = df_int_non_edu_collab, aes(x = organisation_name, y = count, fill = colour)) +
  ...
  scale_y_continuous(name = NULL, 
                     ...
                     expand = c(0, 0)) +
  scale_fill_identity(name = "Type of research organisation",
                    labels = df_int_non_edu_collab$organisation_type,
                    breaks = df_int_non_edu_collab$colour,
...
df_uk_orgs <- research_organisations %>%
  ...
  scale_y_continuous(name = NULL, 
                     ...
                     expand = c(0, 0)) +
  scale_fill_identity(name = "Type of research organisation",
                    labels = df_uk_orgs$organisation_type,
                    breaks = df_uk_orgs$colour,
                    guide = (guide_legend(title.position = "top", nrow = 2, byrow = FALSE))) +
  coord_flip() +
  theme_light() +
  theme(axis.text.x = element_blank(),
