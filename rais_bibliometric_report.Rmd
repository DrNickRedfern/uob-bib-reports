...
n_researchers_in_results <- n_distinct(researchers$full_name)
...
n_researchers_without_orcid <- researchers %>%
...
ggplot(data = collab_key_stats, 
  ...
  geom_text(colour = "white", fontface = "bold", size = 20,
            aes(label = values, x = x - 2.9, y = y - 1.5), hjust = 0, vjust = 0) +
  # text for tile headings
  geom_text(colour = "white", fontface = "bold", size = 5.5, lineheight = 0.9,
            aes(label = text, x = x - 2.9, y = y + 1.4), hjust = 0) +
  coord_fixed() +
  scale_fill_manual(name = NULL, values = discrete_pal[c(5, 6, 8, 9)]) +
  theme(plot.margin = unit(c(-0.30, 0 , 0, 0), "null")) +  # remove margin around plot
...
outputs_per_altmetric <- df_altmetric_types %>%
  ...
  tally(name = "outputs") %>%
  ungroup() %>%
  mutate(percent = round(100 * outputs/n_unique_outputs, 1))
...
Table \@ref(tab:tbl-altmetric-summary) breaks down research outputs by channel of online attention, showing the number of outputs produced by researchers from the `r params$research_unit` that received attention via a particular channel and the total number of mentions received via that channel. Research outputs received attention across a total of `r dim(altmetric_summary)[1] - 1` differnt channels of attention, excluding Mendeley readers.
